...
var controllers = function() {
      ...
      function makeNewRoute() {
        reRoutes.count({}, function(err, data) {
          if (err) {
            ...
            console.log(data);
            var count = data;
            reRoutes.insert({'originalurl': givenRoute, 'shorturl': count},
                            function(err, data) {
              if (err) {
                throw new Error('makeNewRoute insert failed');
              } else {
                var newRoute = {'originalurl': data.ops[0].originalurl,
                                'shorturl': 'https://fcc-apis-fugalfunkster.c9users.io/api/shorturl/' + data.ops[0].shorturl};
                res.send(newRoute);
              }
            });
          }
  ...
  var imageSearchController =  function(req, res) {
    mongo.connect('mongodb://localhost:27017/api', function(err, db) {
      ...
      var results = [];
      searchDB.find({}, projection).sort([['when',1]]).limit(10).toArray(
          function(err, docArray) {
        if (err) {
          console.log(err);
        }
